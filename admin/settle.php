...
while($row = $rs->fetch())
{
	...
	$DB->exec("INSERT INTO `pay_settle` (`pid`, `batch`, `type`, `note`, `username`, `account`, `money`, `fee`, `time`, `status`) VALUES ('{$row['id']}', '{$batch}', '{$row['type']}', '{$row['note']}', '{$row['username']}', '{$row['account']}', '{$row['money']}', '{$fee}', '{$date}', '1')");
	$allmoney+=$row['money'];
}
