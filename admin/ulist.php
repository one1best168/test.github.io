...
<label>请选择收款方式:</label><br><select class="form-control" name="note">
...
<div class="form-group">
<label>请选择收款方式:</label><br><select class="form-control" name="note" default="'.$row['note'].'">
<option value="支付宝">支付宝</option>
<option value="QQ钱包">QQ钱包</option>
<option value="微信">微信</option>
</select>
...
$key = random(32);
$sds=$DB->exec("INSERT INTO `pay_user` (`key`, `note`, `qq`, `account`, `username`, `money`, `url`, `addtime`, `type`, `active`) VALUES ('{$key}', '{$note}', '{$qq}', '{$account}', '{$username}', '{$money}', '{$url}', '{$date}', '{$type}', '{$active}')");
$pid=$DB->lastInsertId();
if($sds){
...
$sql="update `pay_user` set `note` ='{$note}',`qq` ='{$qq}',`account` ='{$account}',`username` ='{$username}',`money` ='{$money}',`url` ='{$url}',`type` ='$type',`active` ='$active' where `id`='$id'";
if($_POST['resetkey']==1){
	$key = random(32);
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>商户号</th><th>密钥</th><th>余额</th><th>结算方式/结算账号/姓名</th><th>域名/添加时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
