...
while($res = $rs->fetch())
{
echo '<tr><td><b>'.$res['id'].'</b></td><td>'.$res['pid'].'</td><td>'.$res['type'].'</td><td>'.$res['account'].'&nbsp;'.$res['username'].'</td><td><b>'.$res['money'].'</b>&nbsp;/&nbsp;<b>'.$res['fee'].'</b></td><td>'.$res['time'].'</td><td>'.($res['status']==1?'<font color=green>已出款</font>':'<font color=blue>未出款</font>').'</td></tr>';
}
?>
          </tbody>
        </table>
