...
elseif($act=='query')
{
	...
	if($row){
		if($key==$row['key']){
			$result=array("code"=>1,"pid"=>$pid,"key"=>$key,"type"=>$type,"active"=>$row['active'],"money"=>$row['money'],"account"=>$row['account'],"username"=>$row['username'],"settle_money"=>$conf['settle_money'],"settle_fee"=>$conf['settle_fee'],"money_rate"=>$conf['money_rate']);
		}else{
...
elseif($act=='change')
{
	...
	if($row){
		if($key==$row['key']){
			if($row['type']!=2 && !empty($row['account']) && !empty($row['username']) && strlen($row['username'])>3 && $row['account']!=$account && (strpos($row['account'],'@') || strlen($row['account'])==11)){
				$result=array("code"=>-1,"msg"=>"为保障您的资金安全，暂不支持直接修改结算账号信息，如需修改请联系QQ1277180438");
			}else{
				$type=1;
				$sds=$DB->exec("update `pay_user` set `account`='{$account}',`username`='{$username}',`type`='{$type}',`url`='{$url}' where id='{$pid}' limit 1");
				if($sds>=0){
